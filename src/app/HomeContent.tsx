...
import deskPartner1 from "@/assets/desk-partner-1.png";
import deskPartner2 from "@/assets/desk-partner-2.jpg";
import deskPartner3 from "@/assets/desk-partner-3.png";
import { ArrowRight, ArrowUpRight } from "lucide-react";

// Dynamically import NetworkCanvas to disable SSR for WebGL/Three.js rendering
const NetworkCanvas = dynamic(
  () => import("@/components/site/NetworkCanvas").then((mod) => mod.NetworkCanvas),
  { ssr: false }
);
...
const partners = [
  { name: "Rashida Adenwala", role: "Founder Partner", img: partner1, pos: "object-center" },
  { name: "Ramakrishna Gupta Racharla", role: "Senior Partner", img: partner2, pos: "object-[80%_center]" },
  { name: "B. Bhavani Raj", role: "Partner", img: partner3, pos: "object-center" },
];

const introSlides = [
  { img: deskPartner1, pos: "object-center" },
  { img: deskPartner2, pos: "object-[80%_center]" },
  { img: deskPartner3, pos: "object-center" },
];
  ...
  return (
    <SiteLayout>
      ...
      <section className="relative min-h-[100svh] flex items-end overflow-hidden bg-background pt-16">
        ...
        <div className="relative z-10 mx-auto max-w-[1400px] w-full px-6 lg:px-10 pb-24 lg:pb-32">
          ...
          <h1 className="font-display text-[clamp(3rem,9vw,9rem)] leading-[0.95] font-medium text-balance">
            ...
            <SplitReveal as="span" delay={0.15} className="block italic text-accent">Incorporated.</SplitReveal>
          </h1>
          <div className="mt-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
            <p className="lg:col-span-6 text-base lg:text-lg text-muted-foreground max-w-xl text-balance">
              Defining Trustworthiness and Lasting Relationships
            </p>
            <div className="lg:col-span-6 flex flex-wrap gap-3 lg:justify-end">
              <Link
                href="/contact"
                className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-6 py-3 text-xs uppercase tracking-[0.18em] hover:bg-accent hover:text-accent-foreground hover:gap-3 transition-all duration-300"
              >
                Initiate Consultation <ArrowRight className="h-3.5 w-3.5" />
              </Link>
              <Link
                href="/what-we-do"
                className="inline-flex items-center gap-2 border border-primary px-6 py-3 text-xs uppercase tracking-[0.18em] hover:bg-accent hover:border-accent hover:text-accent-foreground transition-all duration-300"
              >
                Explore Practice
              </Link>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
            ...
            <div className="aspect-[4/5] overflow-hidden bg-muted relative">
              {introSlides.map((slide, idx) => (
                <Image
                  key={idx}
                  src={slide.img}
                  alt={`R&A Partner Desk ${idx + 1}`}
                  unoptimized
                  priority
                  className={`absolute inset-0 h-full w-full object-cover ${slide.pos} transition-opacity duration-1000 ease-in-out ${
          ...
          <div className="lg:col-span-7 lg:pt-12">
            <p className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground mb-6">
              — Over 30 years since the inception of R&amp;A Associates
            </p>
              ...
              <div key={p.name} className="group">
                <div className="relative aspect-[4/5] overflow-hidden bg-muted">
                  <Image
                    ...
                    alt={p.name}
                    placeholder="blur"
                    className={`h-full w-full object-cover transition-all duration-[1200ms] ease-out group-hover:scale-[1.02] ${p.pos}`}
                  />
                </div>
      ...
      <section className="relative overflow-hidden min-h-[400px] flex items-center justify-center">
        ...
        />
        <div className="relative z-10 mx-auto max-w-[1400px] px-6 lg:px-10 py-32 lg:py-40 text-center">
          ...
          <Link
            href="/contact"
            className="mt-10 inline-flex items-center gap-2 bg-primary text-primary-foreground px-8 py-4 text-xs uppercase tracking-[0.2em] hover:bg-accent hover:text-accent-foreground hover:gap-3 transition-all duration-300"
          >
